...
Dont worry, it's easy. Just go to {bot.get_channel(1031644670436061327).mention} and click the green button.
...
#-------------------------------#
	...
	async def on_member_remove(self, member):
		if 'Needs Approval' in [role.name for role in member.roles]:
			...
			logs_channel = await bot.fetch_channel(1043967844934762556) # Welcome channel
...
#-------------------------------#
	...
	async def on_member_update(self, before, after):
		if before.nick != after.nick and before.nick is not None:
			...
			logs_channel = await bot.fetch_channel(1044331443884654692) # ADMIN user log
			await logs_channel.send(embed=embed)
		...
		# Verification success logging	
			...
			logs_channel = await bot.fetch_channel(1043967844934762556) # user
